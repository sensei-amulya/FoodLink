...
export const addFood = async (req, res) => {
  try {
    const { name, quantity, longitude, latitude, expiryTime, type, image } =
      req.body;
    ...
    // Update donor's location to match the food donation location
...
export const getNearbyFood = async (req, res) => {
  try {
    ...
    // Update receiver's location to their current position
    await User.findByIdAndUpdate(req.user._id, {
      location: {
        type: "Point",
    ...
    const foodListing = await Food.find(query).populate(
      "donorId",
      "name rating",
    );
...
export const claimFood = async (req, res) => {
  try {
    ...
    // Update receiver's location if provided
...
export const updateFoodStatus = async (req, res) => {
  try {
    ...
    const isReceiver =
      food.receiverId && food.receiverId.toString() === req.user._id.toString();
...
export const getReceiverListings = async (req, res) => {
  try {
    if (req.user.role !== "Receiver" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Receivers can access their claims" });
    }
...
export const getDonorListings = async (req, res) => {
  try {
    if (req.user.role !== "Donor" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Donors can access their listings" });
    }
...
export const acceptDelivery = async (req, res) => {
  try {
    ...
    if (food.deliveryStatus && food.deliveryStatus !== "pending") {
      return res
        .status(400)
        .json({ message: "Delivery already accepted by another volunteer" });
    }
...
export const markPickedUp = async (req, res) => {
  try {
    ...
    if (food.donorId.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the donor can mark this as picked up" });
    }
    if (food.deliveryStatus !== "accepted")
      return res.status(400).json({ message: "Delivery not accepted yet" });
...
export const markDelivered = async (req, res) => {
  try {
    ...
    if (!deliveryProofImage)
      return res
        .status(400)
        .json({ message: "Delivery proof image is required" });

    const food = await Food.findById(req.params.id);
    if (!food) return res.status(404).json({ message: "Food not found" });
    if (food.volunteerId.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: "Only the assigned volunteer can mark this as delivered",
      });
    }
    if (!food.pickupConfirmed)
      return res
        .status(400)
        .json({ message: "Cannot deliver before pickup is confirmed" });
...
export const confirmDelivery = async (req, res) => {
  try {
    ...
    if (food.donorId.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the donor can confirm final delivery" });
    }
    if (food.deliveryStatus !== "delivered")
      return res
        .status(400)
        .json({ message: "Food has not been marked delivered yet" });
...
export const editFood = async (req, res) => {
  try {
    ...
    if (food.donorId.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: "Only the original donor can edit this food listing",
      });
    }

    if (food.status !== "Available") {
      return res.status(400).json({
        message: "Can only edit food listings that are currently Available",
      });
    }
...
export const deleteFood = async (req, res) => {
  try {
    ...
    if (food.donorId.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: "Only the original donor can delete this food listing",
      });
    }
...
export const getCompostAvailable = async (req, res) => {
  try {
    if (req.user.role !== "Farmer" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Farmers can access compost listings" });
    }

    const { lng, lat, distance = 50000 } = req.query; // default 50km for farmers

    if (lng && lat) {
      // Update farmer's location to their current position
      await User.findByIdAndUpdate(req.user._id, {
        location: {
          type: "Point",
    ...
    const compostListings = await Food.find(query).populate(
      "donorId",
      "name address location rating email",
    );
...
export const claimCompost = async (req, res) => {
  try {
    if (req.user.role !== "Farmer" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Farmers can claim compost" });
    }
    ...
    if (!food.isCompostable || food.compostStatus !== "available") {
      return res
        .status(400)
        .json({ message: "Food is not available for compost" });
    }
...
export const markCompostCollected = async (req, res) => {
  try {
    if (req.user.role !== "Farmer" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Farmers can mark compost as collected" });
    }
    ...
    if (food.farmerId?.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        message: "Only the claiming farmer can mark this as collected",
      });
    }
...
export const getMyCompostClaims = async (req, res) => {
  try {
    if (req.user.role !== "Farmer" && req.user.role !== "Admin") {
      return res
        .status(403)
        .json({ message: "Only Farmers can access their claims" });
    }
...
export const markExpiredForCompost = async (req, res) => {
  try {
    ...
    if (food.donorId.toString() !== req.user._id.toString()) {
      return res
        .status(403)
        .json({ message: "Only the original donor can expire this food" });
    }
    if (food.status !== "Available") {
      return res
        .status(400)
        .json({ message: "Only Available food can be manually expired" });
    }
