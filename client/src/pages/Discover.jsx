...
const Discover = () => {
  ...
  const confirmClaim = async () => {
    ...
    try {
      ...
      await api.put(`/food/${claimingFood._id}/claim`, locationData);
  ...
  return (
    <div className="h-[calc(100vh-64px)] w-full flex flex-col bg-gray-50 relative overflow-hidden">
              ...
              <div>
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                  {["all", "veg", "non-veg"].map((t) => (
                    <button
                      key={t}
                      onClick={() => applyFilters({ ...filters, type: t })}
                      className={`flex-1 py-1.5 text-xs font-bold rounded-lg capitalize transition-colors ${filters.type === t ? "bg-white text-green-600 shadow-sm" : "text-gray-500"}`}
                    >
                      {t}
                    </button>
                  ))}
              ...
              <div>
                <label className="flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      applyFilters({ ...filters, urgentOnly: e.target.checked })
                    }
                    className="rounded text-green-500 focus:ring-green-500 mr-3 w-5 h-5 accent-green-500"
                  />
                  <span className="text-sm font-semibold text-gray-700">
                    Urgent Only (Expires &lt; 2 hrs)
                  </span>
                </label>
              </div>
            </div>
      ...
      <div className="flex-1 relative z-10 w-full h-full">
                  ...
                  <Popup>
                    <div className="min-w-[200px] p-1 font-sans">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-extrabold text-gray-900 border-b pb-1 truncate block max-w-[150px]">
                          {food.name}
                        </h3>
                        <span
                          className={`px-2 py-0.5 rounded text-[10px] font-bold text-white ${food.type === "veg" ? "bg-green-500" : "bg-red-500"}`}
                        >
                          {food.type === "veg" ? "Veg" : "Non-Veg"}
                        </span>
                      </div>

                      <p className="text-xs mb-1.5 flex justify-between">
                      ...
                      <p
                        className={`text-xs mb-3 flex justify-between font-bold ${food.isUrgent ? "text-red-500" : "text-orange-500"}`}
                      >
                        <span>Expires in:</span>
                        <span>
                          {food.timeRemaining.hours}h{" "}
                          {food.timeRemaining.minutes}m
                        </span>
                      </p>

                      <button
                        onClick={() => setClaimingFood(food)}
                        className="w-full bg-green-500 hover:bg-green-600 text-white text-xs font-bold py-2 rounded-lg shadow-md shadow-green-500/20 active:scale-95 transition-all"
                      >
                        Claim Food
                      </button>
                    </div>
        ...
        {/* List View */}
        <div
          className={`w-full h-full bg-gray-50 overflow-y-auto p-4 sm:p-6 lg:p-8 ${viewMode === "list" ? "block" : "hidden"}`}
        >
            ...
            {processedFoods.length === 0 && !loading && (
              <div className="text-center py-20 bg-white rounded-3xl border border-gray-100 shadow-sm mt-10">
                ...
                <h3 className="text-xl font-bold text-gray-800 mb-2">
                  No food found in this area
                </h3>
                <p className="text-gray-500">
                  Try expanding your distance filter or changing food types.
                </p>
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="h-40 bg-gray-100 relative group-hover:scale-105 transition-transform duration-500">
                      ...
                      <div className="absolute top-3 left-3 flex items-center shadow-lg">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-bold text-white shadow-sm flex items-center ${food.type === "veg" ? "bg-green-500" : "bg-red-500"}`}
                        >
                          {food.type === "veg" ? (
                    ...
                    <div className="p-6 relative bg-white z-10">
                      <h3 className="font-extrabold text-xl text-gray-900 mb-4 truncate group-hover:text-green-500 transition-colors">
                        {food.name}
                      </h3>
                      ...
                      <div className="space-y-3 mb-6">
                        ...
                        <div className="flex justify-between items-center text-sm">
                          ...
                          <span
                            className={`font-bold pl-2 py-1 rounded-md ${food.isUrgent ? "text-red-500" : "text-orange-500"}`}
                          >
                            {food.timeRemaining.hours}h{" "}
                            {food.timeRemaining.minutes}m
                          </span>
                      ...
                      <button
                        onClick={() => setClaimingFood(food)}
                        className="w-full bg-green-50 hover:bg-green-500 text-green-600 hover:text-white border border-green-100 py-3 rounded-xl font-bold transition-all flex justify-center items-center shadow-sm hover:shadow-green-500/20 active:scale-95"
                      >
