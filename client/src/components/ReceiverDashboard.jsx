...
const ReceiverDashboard = () => {
  ...
  const fetchNearbyFood = useCallback(async (lat, lng) => {
    try {
      const { data } = await api.get(`/food/nearby?lat=${lat}&lng=${lng}&distance=500000`);
      setFoods(data);
      setError(null);
    } catch (err) {
  ...
  // ─── 500 ms polling ───────────────────────────────────────────────────────
  // Poll myClaims always; poll nearby food only once we have the user location
                  ...
                  {claim.deliveryStatus && claim.deliveryStatus !== 'pending' && (
                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200 mb-5 shadow-sm">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <span className="text-xs font-extrabold text-gray-500 block uppercase tracking-wide mb-1">Delivery Partner</span>
                          <span className="text-base text-gray-900 font-bold">{claim.volunteerName}</span>
                        </div>
                        <span className={`px-3 py-1.5 text-xs font-extrabold tracking-widest uppercase rounded-lg ${claim.deliveryStatus === 'completed' ? 'bg-green-100 text-green-700 border-green-200 shadow-sm' : 'bg-blue-100 text-blue-700 border-blue-200 shadow-sm'}`}>
                          {claim.deliveryStatus === 'completed' ? 'COMPLETED' : 'ASSIGNED'}
                        </span>
                      </div>
                      <span className="text-gray-700 text-sm font-extrabold">📞 {claim.volunteerPhone}</span>
                    </div>
                  )}
